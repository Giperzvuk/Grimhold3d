...
const R = (() => {
  ...
  function treeField(kind, list) { // list: [{x,z,s}]
    ...
    base.thinInstanceSetBuffer('matrix', mats, 16, true); base.thinInstanceEnablePicking = false;
    // sharedMat обязателен: материал дерева приходит из кэша Models.colMat и общий
    // для всех уровней. Без флага разборка уровня зовёт dispose(false, true) и вправе
    // уничтожить его, а кэш продолжит отдавать мёртвый материал следующему уровню.
    // decorField рядом помечен так же — treeField просто забыли.
    base.metadata = { list, kind, sharedMat: true }; return base;
  }
