...
const UI = {
  ...
  drawHud(dt) {
    ...
    // --- подсказки-обучение ---
    // Порядок важен: показывается первая незакрытая подсказка, поэтому шаг, который
    // нельзя выполнить прямо сейчас, останавливает всю цепочку. Парирование требует
    // врага с замахом, а в стартовой деревне врагов нет — поэтому оно идёт последним
    // и закрывается при первой же стычке, не загораживая остальное обучение.
    if (!this.open && !P.dead) { const hints = IN.touch
      ? [['move', 'Левый палец — джойстик. Да, вот так. Молодец'], ['touch', 'Правой половиной экрана — обзор. Мир большой, в основном пустой'], ['use', 'Кнопка ДЕЙСТВИЕ: говорить, открывать, брать чужое'], ['attack', 'УДАР — атака. Удержать — блок. Блок работает лучше, чем кажется'], ['spell', 'ОГОНЬ — заклинание. Удержать — сменить'], ['quick', 'Ячейки 1–4: тап — выпить, удержать — очистить'], ['inv', 'Сумка вверху справа. Там лежит всё, что ты у кого-то забрал'], ['craft', 'Наковальня, печь и котёл — ремесло. Дешевле, чем у Брандта, и злее'], ['parry', 'Блок ровно в момент замаха — парирование. Враг отлетит и обидится']]
      : [['move', 'WASD — ходьба, мышь — обзор'], ['use', 'E — говорить, открывать, брать чужое'], ['attack', 'ЛКМ — удар, ПКМ — блок'], ['spell', 'F — заклинание, колесо — сменить'], ['quick', '1–4 — быстрые ячейки'], ['inv', 'I — сумка, J — дневник, M — карта'], ['craft', 'Наковальня, печь и котёл — ремесло'], ['parry', 'Блок ровно в момент замаха — парирование']]; const h = hints.find(h => !G.hints[h[0]]); if (h) { const hy = cy0 + 24; c.font = 'bold 14px "Courier New", monospace'; c.textAlign = 'center'; c.fillStyle = 'rgba(0,0,0,.6)'; const tw = c.measureText(h[1]).width + 24; c.fillRect(W / 2 - tw / 2, hy, tw, 26); c.fillStyle = '#ffd060'; c.fillText(h[1], W / 2, hy + 13); c.textAlign = 'left'; c.font = 'bold 13px "Courier New", monospace'; } }
    // --- виньетки ---
