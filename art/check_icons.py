...
def check_light(a, solid, r):
    """§A.3: запечённые тени и направленный свет запрещены — освещение в игре своё."""
    l = lum(a[..., :3].astype(np.float64))
    ys, xs = np.where(solid)
    if len(ys) < 64:
        return
    ...
    if max(gv, gh) > LIGHT_GRAD_PCT:
        r.note(f'похоже на направленный свет: половины предмета отличаются по яркости на '
               f'{gv:.0f}% сверху вниз и {gh:.0f}% слева направо при пороге {LIGHT_GRAD_PCT}%')
    else:
        r.ok(f'освещение ровное (перепад {gv:.0f}% / {gh:.0f}%)')
